...
if [ -n "${WERCKER_PIP_INSTALL_ALLOW_UNVERIFIED}" ]; then
...
if [ -n "${WERCKER_PIP_INSTALL_ALLOW_EXTERNAL}" ]; then
    info 'Allow external packages: $WERCKER_PIP_INSTALL_ALLOW_EXTERNAL'
    export PIP_ALLOW_EXTERNAL="$WERCKER_PIP_INSTALL_ALLOW_EXTERNAL"
fi
...
if [ -n "${WERCKER_PIP_INSTALL_PACKAGES_LIST}" ]; then
